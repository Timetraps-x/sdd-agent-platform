...
- Graph handoff 从 source localization metadata 升级为 harness metadata，并在 Phase 8 code graph 中消费。
...
| 5.6 Phase 7 Graph Handoff Hardening | graph-ready harness metadata | 只稳定 Phase 8 输入，不实现图谱；避免 Phase 5 scope creep。 |
...
- 5.6 收口 graph-ready metadata，作为 Phase 8 前置；新的 Phase 6 会先补充 agent / skill runtime metadata，Phase 7 会先收敛 core module boundary。
...
- Phase 5.6：验证 metadata schema 被 Phase 8 artifact 消费，不实现 graph runtime。
