...
Phase 7.0 是原 Phase 6.0 Code Knowledge Graph Baseline 的顺延版本。它在 Phase 6 Agent / Skill / Team Runtime Harness、Phase 6.1 Resident Agent Worker Runtime、Phase 6.2 RC Stabilization、Phase 6.3 Declarative Agent/Skill Capability Runtime、Phase 6.4 Spec Partition Entry、Phase 6.5 Parallel Branch Run Isolation、Phase 6.6 Documentation Information Architecture、Phase 6.7 Token Budget and Output Dedup Runtime、Phase 6.8 Project Document Language Runtime、Phase 6.9 Runtime Trust Layer and Fast Path Hardening 和 Phase 6.10 Context Budget Runtime and Non-authoritative Log Workers 完成后再启动，消费 Phase 5 的 harness metadata、Phase 5.10 的 document-chain evidence、Phase 6 形成的 agent / skill / team / host adapter execution records、Phase 6.2 的 release-candidate validation evidence、Phase 6.3 的 merged runtime registry evidence、Phase 6.4 的 partition/revision evidence、Phase 6.5 的 partition-aware run isolation evidence、Phase 6.6 的 documentation IA evidence、Phase 6.7 的 output dedup evidence、Phase 6.8 的 document language evidence、Phase 6.9 的 CER claim、PROV-like provenance graph、SddEvidenceAttestation、PolicyDecision 与 runtime trust / fast-path evidence，以及 Phase 6.10 的 context profile、hash-backed evidence summary、context build package、command output summary 和 non-authoritative log worker boundary。
...
- 消费 Phase 6.2 的 RC validation evidence、Phase 6.3 的 merged runtime registry evidence、Phase 6.4 的 partition/revision evidence、Phase 6.5 的 partition-aware run isolation evidence、Phase 6.6 的 documentation IA evidence、Phase 6.7 的 output dedup evidence、Phase 6.8 的 document language evidence、Phase 6.9 的 policy-proven runtime trust / fast-path evidence，以及 Phase 6.10 的 context budget / non-authoritative projection evidence，确保 graph baseline 基于稳定且可扩展的 agent/skill/source registry、spec namespace、run provenance、文档分类边界、runtime 输出边界、文档语言边界、证据质量、policy-backed acceptance coverage、provenance graph、attestation materials、delegation routing、invocation ledger、sync-back 状态、命令 profiling、context package 和非权威摘要边界启动。
...
- 不在 Phase 7.0 之前引入 graph database 或 embedding store 作为平台硬依赖。
...
| TaskRunEvidence | `.sdd/runs` state/events/artifacts | run/evidence provenance graph |
...
| AgentExecutionRecord | `.sdd/runs/<run_id>/agent-executions` | actual agent execution provenance graph |
...
- 能消费 Phase 6.2 的 RC validation evidence、Phase 6.3 的 merged runtime registry evidence、Phase 6.4 的 partition/revision evidence、Phase 6.5 的 partition-aware run isolation evidence、Phase 6.6 的 documentation IA evidence、Phase 6.7 的 output dedup evidence、Phase 6.8 的 document language evidence、Phase 6.9 的 runtime trust / fast-path evidence，以及 Phase 6.10 的 context budget / non-authoritative projection evidence，确认 graph baseline 启动时 core/CLI/test/package 边界、agent/skill/source 扩展机制、spec namespace、run provenance、文档分类边界、runtime 输出边界、文档语言边界、policy-proven PASS 边界、provenance graph、attestation materials、invocation provenance、sync-back 单调状态、fast-path 正确性、context package 和摘要非权威边界已通过验证。
...
- 后续 `specs/master/phase7.0-spec.md`
- 后续 `specs/master/phase7.0-plan.md`
- 后续 `specs/master/phase7.0-tasks.md`
- 后续 `specs/master/phase7.0-validation.md`