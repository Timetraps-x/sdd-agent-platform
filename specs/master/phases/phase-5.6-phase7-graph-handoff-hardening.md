...
Phase 5.6 稳定未来 Phase 8 Code Knowledge Graph Baseline 的 Phase 5 harness metadata 输入。它只做 graph-ready harness metadata handoff，不实现 graph database、embedding、AST/LSP graph 或 graph query runtime。

新的 Phase 6 Agent / Skill Runtime Harness 会在图谱前补充 agent / skill / host adapter runtime metadata；Phase 7 Core Runtime Modularization 会在图谱前收敛 core module boundary；Phase 5.6 只定义 Phase 8 仍需消费的 Phase 5 harness metadata 边界。
...
- 定义 Phase 8 消费的 graph-ready harness metadata schema。
- 更新 Phase 8 artifact/spec 输入边界，使 Phase 8 依赖 Phase 5.6 handoff、Phase 6 runtime metadata 和 Phase 7 core boundary metadata，而不是从自由文本反推 Phase 5/6/7 语义。
...
| Metadata | Fact source | Required fields | Phase 8 usage |
|---|---|---|---|
...
| AgentFit | task graph metadata and AgentRegistryContract | task_id, candidate_agents, selected_agent, non_use_reason, stop_condition | 让 Phase 8 能解释任务为何适合 scout/planner/implementer/reviewer/validator |
...
- Phase 5.6 只稳定 metadata handoff；graph database、embedding、AST/LSP graph、query dashboard 进入 Phase 8 或更后续拆分。
...
- Phase 8 artifact 明确消费 Phase 5 harness metadata，并依赖 Phase 5.6 handoff。
- Phase 5 不实现图谱存储、embedding、AST/LSP graph 或 query runtime。
- graph-ready metadata schema 足以让 Phase 8 拆分 executable phases。
