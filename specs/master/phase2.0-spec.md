...
- 不引入 Phase 4 npm distribution、Phase 5 harness engineering、Phase 6 agent/skill runtime harness、Phase 7 core modularization 或 Phase 8 代码知识图谱。
...
```
注：早期 Phase 2.0 曾包含独立 init 子入口、根 command 入口，并曾投影 `update` / `instructions` 维护入口；当前形态已改为 `/sdd` 根 skill + lifecycle slash + `/sdd:doctor` 诊断入口，`sdd init`、`sdd update`、`sdd instructions <action>` 保持 CLI/core 能力。
