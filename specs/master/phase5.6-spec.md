...
Phase 8 需要消费 Phase 5 的 harness metadata，但如果 handoff 不稳定，未来图谱会从自由文本反推结构。Phase 5.6 只稳定图谱输入，不提前实现图谱；新的 Phase 6 会在图谱前补充 agent / skill runtime metadata，Phase 7 会先收敛 core module boundary。

## Requirements

- FR-1: 必须定义 Phase 8 可消费的 harness metadata schema。
- FR-2: 必须更新 Phase 8 artifact，使其依赖 Phase 5.6 handoff。
...
- AC-1: Phase 8 输入 metadata 列表完整。
- AC-2: Phase 8 artifact/spec 可基于 metadata 拆分后续 executable phases。
