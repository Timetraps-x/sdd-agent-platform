...
用 graph-ready metadata 为 Phase 8 代码知识图谱预留演进空间，并在 Phase 7 先收敛 core 模块边界
```

第一阶段必须先完成 lifecycle decision model 调研、架构基线和 runtime 骨架，再跑通一个真实任务的规格、计划、任务、实现、审查、验证和回写建议闭环；第二阶段产品化全局 CLI、项目 init、AI 工具入口投影、update/doctor 和 instruction API；第三阶段扩展工具、插件、并发和隔离执行；第四阶段补齐 npm published package 分发基线；第五阶段重构为基于 Claude Code 等 AI 工具的 SDD Harness Engineering，定义 context、risk gate、workflow gate、agent registry、task evidence、query/status、eval 和 learning contract；Phase 6 建设 SDD-controlled Agent / Skill Runtime Harness，引入 profile、router、host adapter、execution record、evidence ingestion 和 reuse-first capability catalog；Phase 7 先进行 Core Runtime Modularization，把 `packages/core/src/index.ts` 拆成稳定 runtime domain modules；Phase 8 再建设项目代码知识图谱。
...
原因：代码知识图谱已顺延为 Phase 8 方向。Phase 1/2/3/4/5 必须先产出 graph-ready metadata、harness contract metadata 和可复用分发证据；Phase 6 再补齐 agent / skill runtime metadata；Phase 7 收敛 core runtime module boundary，后续才能自然汇入图谱。
...
artifacts/
  ...
  v
Future Layers
Entry Projection Layer -> Tool & Plugin Layer -> Worktree/Concurrency Layer -> Package Distribution Layer -> SDD Harness Engineering Layer -> Agent / Skill / Team Runtime Harness -> Core Runtime Modularization -> Code Knowledge Graph
```
...
| Agent / Skill / Team Runtime Harness Layer | Phase 6 extension | agent profile、category/router、model_policy abstraction、host adapter、skill capability catalog、external material quarantine、tool permission、team-mode policy、delegation wave、execution record、evidence ingestion、reuse-first policy | 消费 Phase 5 harness contracts、task metadata、risk/autonomy/required artifacts、Claude Code/OpenCode/native skill/MCP/tool/team capabilities | 重造 OpenCode/Claude Code/plugin runtime、MCP hub、generic scheduler、team runtime、Phase 8 code graph |
| Core Runtime Modularization Layer | Phase 7 extension | package-local build、explicit core subpath exports、domain façade、CLI import boundary、doctor/router/registry responsibility split、CLI-owned terminal renderers、Phase 8 graph handoff | 消费 Phase 1~6 的 core/CLI/runtime contracts 和当前 monolith 真实边界 | 改变 CLI/runtime 行为、恢复 root barrel、暴露内部深路径、实现代码图谱 |
| Future Graph Layer | Phase 8 extension | code graph、SDD graph、run graph、agent execution graph、team execution graph、impact analysis | 消费 task metadata、events、artifacts、validation、decision records、distribution evidence、harness contract metadata、Phase 6 agent/skill/team runtime metadata、Phase 7 core module boundary evidence | Phase 1~7 前置条件、运行时事实源写入 |
...
|---|---|---|---|---|---|---|
| lifecycle decision | state/events/artifact | core lifecycle contract | command gate / core | CLI / doctor / resume / graph | 1.0 research, 1.1 define, 1.2 record, 1.7 gate | Phase 1.2、1.7、1.9、1.10、Phase 2 instruction API、Phase 6 runtime、Phase 8 graph |
...
| run state | `.sdd/runs/<run_id>/state.json` | runtime core | runtime | CLI / doctor / resume / graph | 1.2 | Phase 1.6、1.8、1.9、Phase 2 instruction API/doctor、Phase 6 runtime、Phase 8 graph |
| event log | `.sdd/runs/<run_id>/events.jsonl` | runtime core | runtime | doctor / resume / audit / graph | 1.2 | Phase 1.6、1.9、1.10、Phase 2 doctor、Phase 6 runtime、Phase 8 graph |
| artifact path | `.sdd/runs/<run_id>/artifacts/*` | runtime artifact contract | runtime / agents | validator / sync-back / graph | 1.2 | Phase 1.6、1.8、1.9、Phase 2 instruction API、Phase 6 evidence ingestion、Phase 8 graph |
| sdd-task | `tasks.md` fenced block | task model contract | tasks command / 用户 | task parser / do / validator / graph | 1.3/1.5 | Phase 1.5、1.7、1.8、Phase 3 concurrency、Phase 6 router、Phase 8 graph |
| sdd-result | artifact fenced block | artifact result contract | agents | artifact validator / doctor / graph | 1.3/1.6 | Phase 1.6、1.8、1.9、Phase 2 doctor、Phase 6 evidence ingestion、Phase 8 graph |
...
| gap | artifact + events | gap contract | any agent / runtime | orchestrator / sync-back / graph | 1.6+ | Phase 1.8、1.9、1.10、Phase 2 doctor、Phase 6 router/evidence、Phase 8 graph |
...
## 19. Phase 8 代码知识图谱接入点

Phase 8 承接原代码图谱 baseline 方向。Phase 1/2/3/4/5 必须为它预留可消费 metadata，其中 Phase 5.6 是 graph-ready harness metadata handoff；Phase 6 补齐 agent / skill / host adapter runtime metadata；Phase 7 先收敛 core module boundary，避免图谱阶段继续扩张 core monolith。
...
规则：Phase 1~7 的新增 metadata 必须优先进入结构化 block、event field、execution record、module boundary 或 artifact index；自然语言解释只能作为补充。Project Context Pack、eval summary 和 learning note 只能作为上下文/回归证据，不能覆盖 specs 或 `.sdd/runs` 的结构化事实。
...
### ADR-006：Phase 8 graph-ready metadata 从 Phase 1 开始预留
