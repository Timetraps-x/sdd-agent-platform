...
第一阶段必须先完成 lifecycle decision model 调研、架构基线和 runtime 骨架，再跑通一个真实任务的规格、计划、任务、实现、审查、验证和回写建议闭环；第二阶段产品化全局 CLI、项目 init、AI 工具入口投影、update/doctor 和 instruction API；第三阶段再扩展工具、插件、并发和隔离执行；第四阶段再建设项目代码知识图谱。
...
原因：代码知识图谱是 Phase 4 方向。Phase 1/2/3 必须先产出 graph-ready metadata，后续才能自然汇入图谱。
...
artifacts/
  ...
  v
Future Layers
Entry Projection Layer -> Tool & Plugin Layer -> Worktree/Concurrency Layer -> Run DB/Dashboard -> Code Knowledge Graph
```
...
| Future Graph Layer | Phase 4 extension | code graph、SDD graph、run graph、impact analysis | 消费 task metadata、events、artifacts、validation、decision records | Phase 1/2/3 前置条件、运行时事实源写入 |
...
|---|---|---|---|---|---|---|
| lifecycle decision | state/events/artifact | core lifecycle contract | command gate / core | CLI / doctor / resume / graph | 1.0 research, 1.1 define, 1.2 record, 1.7 gate | Phase 1.2、1.7、1.9、1.10、Phase 2 instruction API、Phase 4 graph |
...
| run state | `.sdd/runs/<run_id>/state.json` | runtime core | runtime | CLI / doctor / resume / graph | 1.2 | Phase 1.6、1.8、1.9、Phase 2 instruction API/doctor、Phase 4 graph |
| event log | `.sdd/runs/<run_id>/events.jsonl` | runtime core | runtime | doctor / resume / audit / graph | 1.2 | Phase 1.6、1.9、1.10、Phase 2 doctor、Phase 4 graph |
| artifact path | `.sdd/runs/<run_id>/artifacts/*` | runtime artifact contract | runtime / agents | validator / sync-back / graph | 1.2 | Phase 1.6、1.8、1.9、Phase 2 instruction API、Phase 4 graph |
| sdd-task | `tasks.md` fenced block | task model contract | tasks command / 用户 | task parser / do / validator / graph | 1.3/1.5 | Phase 1.5、1.7、1.8、Phase 3 concurrency、Phase 4 graph |
| sdd-result | artifact fenced block | artifact result contract | agents | artifact validator / doctor / graph | 1.3/1.6 | Phase 1.6、1.8、1.9、Phase 2 doctor、Phase 4 graph |
...
| gap | artifact + events | gap contract | any agent / runtime | orchestrator / sync-back / graph | 1.6+ | Phase 1.8、1.9、1.10、Phase 2 doctor、Phase 4 graph |
...
## 18. Phase 4 代码知识图谱接入点

Phase 4 暂不拆分，但 Phase 1/2/3 必须为它预留可消费 metadata。
...
| Metadata | Phase 1 来源 | Phase 4 用途 |
|---|---|---|
...
### ADR-006：Phase 4 graph-ready metadata 从 Phase 1 开始预留
