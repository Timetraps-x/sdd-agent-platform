...
用 graph-ready metadata 为 Phase 9 代码知识图谱预留演进空间，并通过 Phase 8 先收敛 coding runtime 的风险、证据、上下文和 workflow 消费模型
```

第一阶段必须先完成 lifecycle decision model 调研、架构基线和 runtime 骨架，再跑通一个真实任务的规格、计划、任务、实现、审查、验证和回写建议闭环；第二阶段产品化全局 CLI、项目 init、AI 工具入口投影、update/doctor 和 instruction API；第三阶段扩展工具、插件、并发和隔离执行；第四阶段补齐 npm published package 分发基线；第五阶段重构为基于 Claude Code 等 AI 工具的 SDD Harness Engineering，定义 context、risk gate、workflow gate、agent registry、task evidence、query/status、eval 和 learning contract；Phase 6 建设 SDD-controlled Agent / Skill Runtime Harness，引入 profile、router、host adapter、execution record、evidence ingestion 和 reuse-first capability catalog；Phase 7 进行 Core Runtime Modularization，把 `packages/core/src/index.ts` 拆成稳定 runtime domain modules；Phase 8 收敛 coding runtime 的 lifecycle、test、sync-back、ship、subagent 和 context 语义；Phase 9 再建设代码图谱信号。
...
原因：代码知识图谱已顺延为 Phase 9 方向。Phase 1/2/3/4/5 必须先产出 graph-ready metadata、harness contract metadata 和可复用分发证据；Phase 6 再补齐 agent / skill runtime metadata；Phase 7 收敛 core runtime module boundary；Phase 8 先稳定 coding runtime 的 risk/context/evidence/workflow 消费模型，后续图谱才能作为可选信号自然接入。
...
Commands / Skills Layer
  /sdd /sdd:spec /sdd:plan /sdd:tasks /sdd:do /sdd:test /sdd:sync-back /sdd:ship /sdd:doctor
  |
  v
...
artifacts/
  ...
  v
Future Layers
Entry Projection Layer -> Tool & Plugin Layer -> Worktree/Concurrency Layer -> Package Distribution Layer -> SDD Harness Engineering Layer -> Agent / Skill / Team Runtime Harness -> Core Runtime Modularization -> Coding Runtime Convergence -> Code Graph Signals
```
...
| Agent / Skill / Team Runtime Harness Layer | Phase 6 extension | agent profile、category/router、model_policy abstraction、host adapter、skill capability catalog、external material quarantine、tool permission、team-mode policy、delegation wave、execution record、evidence ingestion、reuse-first policy | 消费 Phase 5 harness contracts、task metadata、risk/autonomy/required artifacts、Claude Code/OpenCode/native skill/MCP/tool/team capabilities | 重造 OpenCode/Claude Code/plugin runtime、MCP hub、generic scheduler、team runtime、Phase 9 code graph |
| Core Runtime Modularization Layer | Phase 7 extension | package-local build、explicit core subpath exports、domain façade、CLI import boundary、doctor/router/registry responsibility split、CLI-owned terminal renderers、Phase 8 runtime convergence handoff | 消费 Phase 1~6 的 core/CLI/runtime contracts 和当前 monolith 真实边界 | 改变 CLI/runtime 行为、恢复 root barrel、暴露内部深路径、实现代码图谱 |
| Coding Runtime Convergence Layer | Phase 8 extension | verifies-centered lifecycle、risk workflow gates、unified `/sdd:test` evidence judgment、task-scoped sync-back readiness、ship diagnostics/readiness separation、subagent non-authoritative boundary、context offload semantics | 消费 Phase 5 harness metadata、Phase 6 agent/skill/team runtime metadata、Phase 7 storage/resolver/verification/test/sync-back/ship evidence | 实现代码图谱、让图谱信号成为 hard gate、让 subagent 拥有 lifecycle authority |
| Future Graph Layer | Phase 9 extension | code graph signals、SDD graph、run graph、agent execution graph、team execution graph、impact analysis、impacted tests、confidence/reasons | 消费 task metadata、events、artifacts、validation、decision records、harness contract metadata、Phase 6 runtime metadata、Phase 7 core boundary evidence、Phase 8 stabilized risk/context/evidence consumers | Phase 1~8 前置条件、运行时事实源写入、lifecycle authority |
...
packages/
  core/
    package.json          # explicit subpath exports only, no root barrel
    dist/                 # package-local build output
    src/
      status.ts           # public domain façade
      run-state.ts
      sdd-docs.ts
      context.ts
      artifacts.ts
      ...
      runtime-analysis.ts
      test-support.ts
      index.ts            # intentionally not used as old mixed API barrel
      doctor/
        checks/
        model.ts
        summary.ts
  ...
  cli/
    ...
    src/
      ...
      commands/
        registry.ts       # façade
        registry/         # command-family handlers
      renderers/
        registry.ts       # façade
        registry-*.ts     # registry renderer families
        router.ts
        doctor.ts
```

上面的树是 Phase 7.0 的已落地边界：core 通过 `@sdd-agent-platform/core/<domain>` 暴露稳定 domain façade，CLI 不允许相对穿透 `packages/core/src`，root `@sdd-agent-platform/core` barrel 不再作为兼容面。Phase 7C-Final 已收口 doctor check family、router/routing、CLI registry command/renderer，并将 doctor terminal text renderer 迁到 CLI；最终仍用 package/export/dist smoke 防止边界回退。
...
|---|---|---|---|---|---|---|
| lifecycle decision | state/events/artifact | core lifecycle contract | command gate / core | CLI / doctor / resume / graph | 1.0 research, 1.1 define, 1.2 record, 1.7 gate | Phase 1.2、1.7、1.9、1.10、Phase 2 instruction API、Phase 6 runtime、Phase 8 runtime convergence、Phase 9 graph |
...
| runtime state | `.sdd/runtime.sqlite` | runtime core | runtime | CLI / doctor / resume / graph | 1.2, Phase 7+ | Phase 1.6、1.8、1.9、Phase 2 instruction API/doctor、Phase 6 runtime、Phase 8 runtime convergence、Phase 9 graph |
| event ledger | `.sdd/runtime.sqlite` events + projections | runtime core | runtime | doctor / resume / audit / graph | 1.2, Phase 7+ | Phase 1.6、1.9、1.10、Phase 2 doctor、Phase 6 runtime、Phase 8 runtime convergence、Phase 9 graph |
| artifact path | `.sdd/runs/<branchSlug>/evidence/artifacts/*` physical files, `artifacts/*` run-relative refs | runtime artifact contract | runtime / agents | validator / sync-back / graph | 1.2, Phase 8+ | Phase 1.6、1.8、1.9、Phase 2 instruction API、Phase 6 evidence ingestion、Phase 8 unified evidence、Phase 9 graph |
| sdd-task | `tasks.md` fenced block | task model contract | tasks command / 用户 | task parser / do / validator / graph | 1.3/1.5 | Phase 1.5、1.7、1.8、Phase 3 concurrency、Phase 6 router、Phase 8 risk workflow、Phase 9 graph |
| sdd-result | artifact fenced block | artifact result contract | agents | artifact validator / doctor / graph | 1.3/1.6 | Phase 1.6、1.8、1.9、Phase 2 doctor、Phase 6 evidence ingestion、Phase 8 unified evidence、Phase 9 graph |
| delegation | `.sdd/runtime.sqlite` state/events/projections | delegation contract | runtime / command | doctor / liveness / resume | 1.3/1.6, Phase 7+ | Phase 1.6、1.8、1.9、Phase 3 background write |
| gap | artifact + events | gap contract | any agent / runtime | orchestrator / sync-back / graph | 1.6+ | Phase 1.8、1.9、1.10、Phase 2 doctor、Phase 6 router/evidence、Phase 8 workflow readiness、Phase 9 graph |
...
Phase 5 插入在 npm 分发完成之后、Agent / Skill Runtime Harness 和代码知识图谱之前，用来把本项目重构为依托 Claude Code 等 AI tool harness 的 SDD Harness Engineering 层。该层已拆分为 5.0~5.10，其中 5.6 收口 graph-ready harness metadata handoff，5.8~5.10 继续强化 semantic document chain、task contract parser 和 document-chain verify/doctor。
...
| 5.6 | graph-ready harness metadata handoff |
...
## 19. Phase 8 Coding Runtime Convergence 与 Phase 9 图谱接入点

Phase 8 不再承接代码知识图谱实现。它的当前职责是把 Phase 7 已有的 runtime、verification、test、sync-back、ship、team/subagent 和 context/token 能力收敛为统一 coding runtime：风险决定生命周期深度，`/sdd:test` 同时执行命令和判断验收证据，sync-back/ship 只使用正确作用域的 readiness，subagent 只做非授权侧向工作，context offload 成为主上下文压力的可执行处理模型。

Phase 9 承接代码图谱信号。图谱可以消费 Phase 1~8 保留下来的结构化 metadata，但它只能作为 risk、context、test-impact、status 和 doctor 的可选信号源；缺失、过期或低置信图谱信号不能成为 Phase 8 command、status、doctor、ship、sync-back 或 `/sdd:test` 的 hard gate。

### 19.1 Phase 8 稳定的图谱消费者边界

```text
LifecycleRiskDecision  可消费未来 changed refs / dependency fanout / confidence，但不能依赖图谱才能决策
ContextOffloadDecision 可消费未来 impacted modules / fanout，但缺失图谱时必须有现有 context/runtime fallback
UnifiedTestEvidence    可消费未来 impacted tests 建议，但验收 PASS 仍来自 validator evidence 和 policy-backed coverage
StatusDoctorReadiness  可展示未来 graph freshness/confidence，但不能用图谱替代 run/evidence/sync-back facts
SubagentRuntime        可接收 scoped context 建议，但 graph 不能选择 agent、dispatch subagent 或完成 lifecycle gate
```

### 19.2 Phase 9 图谱节点和边的初始范围

Phase 9 可以从历史 baseline 继续保留以下图谱对象：Project、Spec、Plan、Task、Acceptance、File、Symbol、API、Run、Event、Artifact、Gap、ValidationCommand、ValidationEvidence、Decision、HarnessContract、AgentProfileContract、SkillCapabilityContract、AgentExecutionRecord、EvidenceIngestionRecord。

Phase 9 的首批边应聚焦 coding runtime 可消费的信号：Task affects File、Symbol defined_in File、Symbol references Symbol、Artifact proves Acceptance、ValidationCommand verifies Acceptance、Run executes Task、Event belongs_to Run、AgentExecutionRecord executes Task、EvidenceIngestionRecord produces Artifact、Decision changes Contract。

### 19.3 Graph-ready metadata 来源

| Metadata | 上游来源 | Phase 9 用途 |
|---|---|---|
| HarnessContract / WorkflowGateResult | Phase 5 harness contracts | 连接 context/risk/workflow/agent/query/eval/learning contract 节点 |
| TaskGraphNode / TaskRunEvidence | `sdd-task`、run state、events、artifacts | 构建 task dependency、affected files、run/evidence provenance 和 execution history |
| VerificationAvailability / UnifiedTestEvidence | verify contract、test runtime、validator artifacts、acceptance coverage | 建立 acceptance、validation command、evidence、policy decision 的映射 |
| AgentProfileContract / AgentExecutionRecord | Phase 6 agent/skill/team runtime | 连接 agent capability、host adapter、execution provenance 和 evidence ingestion |
| Runtime storage/resolver evidence | Phase 7 storage v2、workflow state resolver、sync-back/ship/status/doctor | 连接 branch/task/run readiness、stale detection 和 recovery guidance |
| Coding runtime decisions | Phase 8 risk workflow、subagent boundary、context offload、ship/sync-back semantics | 为 Phase 9 定义实际消费者，避免 graph-first 反向驱动 lifecycle authority |

规则：Phase 1~8 的新增 metadata 必须优先进入结构化 block、event field、execution record、module boundary 或 artifact index；自然语言解释只能作为补充。Project Context Pack、eval summary 和 learning note 只能作为上下文/回归证据，不能覆盖 specs 或 `.sdd/runs` 的结构化事实。
...
### ADR-006：Graph-ready metadata 从 Phase 1 开始预留，Phase 9 承接代码图谱

决定：task metadata、events、artifact result、validation mapping 需要结构化；Phase 8 先稳定 coding runtime 消费模型，Phase 9 再实现代码图谱信号。

原因：代码知识图谱不应未来重新从自由文本里艰难反推，也不应在 risk/context/evidence/workflow 消费边界稳定前反向成为 lifecycle authority。
