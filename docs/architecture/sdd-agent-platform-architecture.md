...
用 graph-ready metadata 为未来代码知识图谱预留演进空间
```

第一阶段必须先完成 lifecycle decision model 调研、架构基线和 runtime 骨架，再跑通一个真实任务的规格、计划、任务、实现、审查、验证和回写建议闭环；第二阶段产品化全局 CLI、项目 init、AI 工具入口投影、update/doctor 和 instruction API；第三阶段扩展工具、插件、并发和隔离执行；第四阶段补齐 npm published package 分发基线；第五阶段再建设项目代码知识图谱。
...
- Sisyphus-style orchestrator：主会话做 intent gate、delegation、综合判断。
- Explore / Librarian / Oracle 分工。
- background read-only task 保护主上下文。
- delegation prompt 强 contract。
- stale/background task 检测思想。
...
原因：其价值在编排纪律，不在具体 runtime API。Claude Code 环境下应转译为 commands、agents、runtime contract 和 artifacts。
...
原因：代码知识图谱已顺延为 Phase 5 方向。Phase 1/2/3/4 必须先产出 graph-ready metadata 和可复用分发证据，后续才能自然汇入图谱。
...
artifacts/
  ...
  v
Future Layers
Entry Projection Layer -> Tool & Plugin Layer -> Worktree/Concurrency Layer -> Package Distribution Layer -> Run DB/Dashboard -> Code Knowledge Graph
```
...
| Future Graph Layer | Phase 5 extension | code graph、SDD graph、run graph、impact analysis | 消费 task metadata、events、artifacts、validation、decision records、distribution evidence | Phase 1/2/3/4 前置条件、运行时事实源写入 |
...
|---|---|
| spec.md | 需求、边界、验收语义 |
| plan.md | 技术方案、影响面、验证策略 |
| tasks.md | task 拆解、依赖、边界、affected_files、validation |
...
|---|---|---|---|---|---|---|
| lifecycle decision | state/events/artifact | core lifecycle contract | command gate / core | CLI / doctor / resume / graph | 1.0 research, 1.1 define, 1.2 record, 1.7 gate | Phase 1.2、1.7、1.9、1.10、Phase 2 instruction API、Phase 5 graph |
...
| run state | `.sdd/runs/<run_id>/state.json` | runtime core | runtime | CLI / doctor / resume / graph | 1.2 | Phase 1.6、1.8、1.9、Phase 2 instruction API/doctor、Phase 5 graph |
| event log | `.sdd/runs/<run_id>/events.jsonl` | runtime core | runtime | doctor / resume / audit / graph | 1.2 | Phase 1.6、1.9、1.10、Phase 2 doctor、Phase 5 graph |
| artifact path | `.sdd/runs/<run_id>/artifacts/*` | runtime artifact contract | runtime / agents | validator / sync-back / graph | 1.2 | Phase 1.6、1.8、1.9、Phase 2 instruction API、Phase 5 graph |
| sdd-task | `tasks.md` fenced block | task model contract | tasks command / 用户 | task parser / do / validator / graph | 1.3/1.5 | Phase 1.5、1.7、1.8、Phase 3 concurrency、Phase 5 graph |
| sdd-result | artifact fenced block | artifact result contract | agents | artifact validator / doctor / graph | 1.3/1.6 | Phase 1.6、1.8、1.9、Phase 2 doctor、Phase 5 graph |
...
| gap | artifact + events | gap contract | any agent / runtime | orchestrator / sync-back / graph | 1.6+ | Phase 1.8、1.9、1.10、Phase 2 doctor、Phase 5 graph |
...
## 18. Phase 5 代码知识图谱接入点

Phase 5 暂不拆分，但 Phase 1/2/3/4 必须为它预留可消费 metadata。
...
如果 Phase 1 的 task、event、artifact 只是自然语言输出，未来图谱只能做脆弱文本抽取。现在通过 `sdd-task`、`sdd-result`、event type、artifact path、agent version、validation mapping 保留结构化信息，后续可以自然汇入 graph。

### 18.4 Graph-ready metadata 来源

| Metadata | 上游来源 | Phase 5 用途 |
|---|---|---|
| decision record | lifecycle decision state/event/artifact | 连接需求风险、profile 和执行路径 |
| task metadata | `sdd-task` 的 id、depends_on、wave、affected_files、validation、risk | 构建任务依赖和影响范围 |
| run/event metadata | `state.json`、`events.jsonl` | 构建执行历史和失败恢复路径 |
| artifact metadata | `sdd-result`、artifact index、agent/version/status | 追踪证据来源和 agent 表现 |
| validation mapping | validation artifact 的 acceptance mapping | 连接验收点、验证命令和证据 |
| gap metadata | gap artifact + events | 分析阻塞、回流和架构漂移 |

规则：Phase 1/2/3 的新增 metadata 必须优先进入结构化 block、event field 或 artifact index；自然语言解释只能作为补充。

## 19. 为什么这样设计

### 19.1 为什么不是纯 Spec Kit

Spec Kit 适合 SDD 语义产物，但不提供本项目需要的 subagent runtime、artifact validation、liveness、gap recovery 和 project adapter 执行事实源。

### 19.2 为什么不是直接 LangGraph / Temporal
...
### 20.1 Phase 1.2+ dependency prerequisites
...
### ADR-006：Phase 5 graph-ready metadata 从 Phase 1 开始预留
